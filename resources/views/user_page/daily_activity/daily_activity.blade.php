                        ...
                        @foreach($activity as $key => $activityItem)
                            <tr>
                                ...
                                @foreach($dates as $date)  
                                    <td>
                                        @php
                                            $tanggal = Carbon\Carbon::createFromFormat('Y-m-d', now()->format('Y-m-') . $date)->toDateString();
                                            $statusRecord = $userActivity->first(function ($item) use ($activityItem, $tanggal) {
                                                return $item->id_activity === $activityItem->id && $item->created_at->format('Y-m-d') === $tanggal;
                                            });
                                        @endphp
                                        ...
                                        <div id="spinner-{{ $statusRecord->id }}" class="spinner-border spinner-border-sm text-primary d-none" role="status">
            ...
            <div class="table-responsive col-6 mt-3">
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        @foreach($activity as $key => $activityItem)
                        @php
                            $monthly = $monthlyActivity->firstWhere('id_activity', $activityItem->jumlah_aktivitas);
